...
if(isset($_REQUEST['hire'])){
    ...
    $userCity = $row['user_district'];
    $userState = $row['user_state'];
    $userDescription = $row['user_description'];
    $userReferenceName = $row['user_r_name'];
    $userReferenceNo = $row['user_r_number'];

    $sql = "INSERT INTO client(`client_id`, `lawyer_id`, `client_name`, `client_cnic`, `client_gender`, `client_email`, `client_mobile`, `client_address`, `client_city`, `client_state`, `client_description`, `client_reference_name`, `client_reference_no`) VALUES ('$userId', '$lawyerId', '$userName','$userCnic','$userGender', '$userEmail', '$userMobile', '$userAddress','$userCity ','$userState', '$userDescription','$userReferenceName','$userReferenceNo')";
    $result = mysqli_query($conn, $sql);
