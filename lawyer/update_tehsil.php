                ...
                <div class="card border-top border-0 border-4 border-dark">
                    <div class="card-body p-5">
                        <div class="card-title d-flex align-items-center">
                            <h5 class="mb-0 text-dark">UPDATE TEHSIL</h5>
                        </div>
                        <hr>
                        <?php
                        $id=$_GET['upid'];
                        $sql="SELECT * FROM `tehsil` INNER JOIN `province` ON `tehsil`.th_province=`province`.pro_id INNER JOIN `district` ON `tehsil`.th_district=`district`.dc_id WHERE `th_id`='$id'"; 
                        $run=mysqli_query($conn,$sql);
                        $fet1=mysqli_fetch_array($run); 
                        ?>

                        <form class="row g-3" id="data">
                        <input type="hidden" class="form-control" name="th_id" value="<?php echo $fet1['th_id'];?>">
						<div class="col-md-12">
...
?>
                        ...
                        <div class="col-md-12">
                            ...
                            <input type="text" class="form-control" name="th_name" value="<?php echo $fet1['th_name'];?>" id="inputpolice station">
                        </div>

                            <div class="col-12">
                             
                                <button type="submit" id="upda" class="btn btn-dark px-5">Update</button>
                                <a href="tehsil.php" class="btn btn-secondary px-5">Back</a>
                            </div>
                        </form>
              ...
              alert(res);
                if (res == 1) {
                    ...
                    Swal.fire({
                        toast: true,
                        icon: 'warning',
                        title: 'Tehsil has not been Updated',
                        animation: false,
                        position: 'top-right',
                        showConfirmButton: false,
                        timer: 3000,
